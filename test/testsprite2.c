...
void MoveSprites(SDL_WindowID window, SDL_TextureID sprite) {
	int i, n;
	...
	if(cycle_color) {
		...
		SDL_SetTextureColorMod(sprite, 255, (Uint8) current_color, (Uint8) current_color);
	}
	if(cycle_alpha) {
...
int main(int argc, char *argv[]) {
	...
	for (i = 1; i < argc;) {
		...
		if(consumed < 0) {
			fprintf(stderr, "Usage: %s %s [--blend none|mask|blend|add|mod] [--scale none|fast|slow|best] [--cyclecolor] [--cyclealpha]\n", argv[0], CommonUsage(state));
			quit(1);
		}
		i += consumed;
	}
	...
	sprites = (SDL_TextureID *) SDL_malloc(state->num_windows * sizeof(*sprites));
	if(!sprites) {
