	...
	for (map = 0; map < NUM_VGAKEYMAPS; ++map) {
		...
		for (i = 0; i < NR_KEYS; ++i) {
			...
			if(ioctl(fd, KDGKBENT, &entry) == 0) {
				...
				if((KTYP(entry.kb_value) == KT_LATIN) || (KTYP(entry.kb_value) == KT_ASCII) || (KTYP(entry.kb_value) == KT_LETTER)) {
					vga_keymap[map][i] = entry.kb_value;
				}
			}
		}
...
static int detect_imps2(int fd) {
	...
	if(!imps2) {
		...
		if(write(fd, &query_ps2, sizeof(query_ps2)) == sizeof(query_ps2)) {
			...
			} while ((read(fd, &ch, sizeof(ch)) == sizeof(ch)) && ((ch == 0xFA) || (ch == 0xAA)));
...
static void switch_vt(_THIS, unsigned short which) {
	...
	if((ioctl(keyboard_fd, VT_GETSTATE, &vtstate) < 0) || (which == vtstate.v_active)) {
		return;
	}
...
void FB_PumpEvents(_THIS) {
	...
	do {
		if(switched_away) {
			struct vt_stat vtstate;
			SDL_mutexP(hw_lock);
			if((ioctl(keyboard_fd, VT_GETSTATE, &vtstate) == 0) && vtstate.v_active == current_vt) {
				switched_away = 0;
				switch_vt_done(this);
			}
			SDL_mutexV(hw_lock);
