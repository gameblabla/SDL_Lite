...
#define RGB888_RGB332(dst, src) { dst = (Uint8)((((src)&0x00E00000)>>16)| (((src)&0x0000E000)>>11)| (((src)&0x000000C0)>>6));}
...
static void Blit_RGB888_index8(SDL_BlitInfo *info) {
	...
	if(map == NULL) {
		while (height--) {
			...
			DUFFS_LOOP(RGB888_RGB332(*dst++, *src);, width);
#else
			for ( c=width/4; c; --c ) {
		...
		while (height--) {
#ifdef USE_DUFFS_LOOP
			DUFFS_LOOP(RGB888_RGB332(Pixel, *src);
						   *dst++ = map[Pixel];
						   ++src;, width);
#else
			for ( c=width/4; c; --c ) {
...
#define RGB888_RGB555(dst, src) { *(Uint16 *)(dst) = (Uint16)((((*src)&0x00F80000)>>9)| (((*src)&0x0000F800)>>6)| (((*src)&0x000000F8)>>3)); }

#define RGB888_RGB555_TWO(dst, src) { *(Uint32 *)(dst) = (((((src[HI])&0x00F80000)>>9)| (((src[HI])&0x0000F800)>>6)| (((src[HI])&0x000000F8)>>3))<<16)| (((src[LO])&0x00F80000)>>9)| (((src[LO])&0x0000F800)>>6)| (((src[LO])&0x000000F8)>>3); }
...
#ifdef USE_DUFFS_LOOP
	while (height--) {
		DUFFS_LOOP(RGB888_RGB555(dst, src);
					   ++src;
					   ++dst;, width);
		src += srcskip;
		dst += dstskip;
...
#define RGB888_RGB565(dst, src) { *(Uint16 *)(dst) = (Uint16)((((*src)&0x00F80000)>>8)| (((*src)&0x0000FC00)>>5)| (((*src)&0x000000F8)>>3)); }
#define RGB888_RGB565_TWO(dst, src) { *(Uint32 *)(dst) = (((((src[HI])&0x00F80000)>>8)| (((src[HI])&0x0000FC00)>>5)| (((src[HI])&0x000000F8)>>3))<<16)| (((src[LO])&0x00F80000)>>8)| (((src[LO])&0x0000FC00)>>5)| (((src[LO])&0x000000F8)>>3); }
...
#ifdef USE_DUFFS_LOOP
	while (height--) {
		DUFFS_LOOP(RGB888_RGB565(dst, src);
					   ++src;
					   ++dst;, width);
		src += srcskip;
		dst += dstskip;
...
#define RGB888_RGB332(dst, src) { dst = (((src)&0x00E00000)>>16)| (((src)&0x0000E000)>>11)| (((src)&0x000000C0)>>6); }
#endif
...
static void BlitNto1(SDL_BlitInfo *info) {
	...
	if(map == NULL) {
		while (height--) {
#ifdef USE_DUFFS_LOOP
			DUFFS_LOOP(DISEMBLE_RGB(src, srcbpp, srcfmt, Pixel, sR, sG, sB);
						   if(1) {
							   /* Pack RGB into 8bit pixel */
							   *dst = ((sR >> 5) << (3 + 2)) | ((sG >> 5) << (2)) | ((sB >> 6) << (0));
						   }
						   dst++;
						   src += srcbpp;, width);
#else
		...
		while (height--) {
			...
			DUFFS_LOOP(DISEMBLE_RGB(src, srcbpp, srcfmt, Pixel, sR, sG, sB);
						   if(1) {
							   /* Pack RGB into 8bit pixel */
							   *dst = map[((sR >> 5) << (3 + 2)) | ((sG >> 5) << (2)) | ((sB >> 6) << (0))];
						   }
						   dst++;
						   src += srcbpp;, width);
#else
	...
	if(palmap == NULL) {
		...
		while (height--) {
			DUFFS_LOOP({
						   ...
						   if((Pixel & rgbmask) != ckey) {
							   /* Pack RGB into 8bit pixel */
							   *dst = (Uint8) palmap[((sR >> 5) << (3 + 2)) | ((sG >> 5) << (2)) | ((sB >> 6) << (0))];
						   }
						   dst++;
						   src += srcbpp;
					   }, width);
...
#if SDL_ARM_SIMD_BLITTERS
	{ 0x00000F00,0x000000F0,0x0000000F, 4, 0x00FF0000,0x0000FF00,0x000000FF, BLIT_FEATURE_HAS_ARM_SIMD, NULL, Blit_RGB444_RGB888ARMSIMD, NO_ALPHA | COPY_ALPHA },
#endif
	{
		0x0000F800,
		0x000007E0,
...
#if SDL_ARM_SIMD_BLITTERS
	{ 0x000000FF,0x0000FF00,0x00FF0000, 4, 0x00FF0000,0x0000FF00,0x000000FF, BLIT_FEATURE_HAS_ARM_SIMD, NULL, Blit_BGR888_RGB888ARMSIMD, NO_ALPHA | COPY_ALPHA },
#endif
	{
		0x00FF0000,
		0x0000FF00,
...
SDL_loblit SDL_CalculateBlitN(SDL_Surface *surface, int blit_index) {
	...
	if(dstfmt->BitsPerPixel == 8) {
		...
		if((srcfmt->BytesPerPixel == 4) && (srcfmt->Rmask == 0x00FF0000) && (srcfmt->Gmask == 0x0000FF00) && (srcfmt->Bmask == 0x000000FF)) {
		...
		for (which = 0; table[which].dstbpp; ++which) {
			if(MASKOK(srcfmt->Rmask, table[which].srcR) && MASKOK(srcfmt->Gmask, table[which].srcG) && MASKOK(srcfmt->Bmask, table[which].srcB) && MASKOK(dstfmt->Rmask, table[which].dstR) && MASKOK(dstfmt->Gmask, table[which].dstG) && MASKOK(dstfmt->Bmask, table[which].dstB) && dstfmt->BytesPerPixel == table[which].dstbpp && (a_need & table[which].alpha) == a_need && ((table[which].blit_features & GetBlitFeatures()) == table[which].blit_features)) {
				break;
			}
		}
		...
		if(blitfun == BlitNtoN) {  /* default C fallback catch-all. Slow! */
			if(srcfmt->BytesPerPixel == 4 && dstfmt->BytesPerPixel == 4 && srcfmt->Rmask == dstfmt->Rmask && srcfmt->Gmask == dstfmt->Gmask && srcfmt->Bmask == dstfmt->Bmask) {
